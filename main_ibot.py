...
from models.clipmodel import CbalipModel,build_clip
...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='mae_vit_base_patch16', type=str,
        choices=['vit_tiny', 'vit_small', 'vit_base', 'vit_large', 'deit_tiny', 'deit_small',
                 'swin_tiny','swin_small', 'swin_base', 'swin_large','mae_vit_base_patch16', 'mae_vit_large_patch16', 'mae_vit_huge_patch14'],
    ...
    # Misc
    parser.add_argument('--pkl_path', default='/home/lcl_d/wuwentao/maeclip/cew.pkl', type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default="/home/lcl_d/wuwentao/maeclip/0821_premodel_lunkuo", type=str, help='Path to save logs and checkpoints.')
...
def train_ibot(args):
    ...
    #轮廓图像 mean=[0.411, 0.414, 0.417], std=[0.277, 0.278, 0.276]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher,teacher_clip, teacher_without_ddp, ibot_loss,
            data_loader_train, optimizer, device,lr_schedule, wd_schedule, momentum_schedule,
            epoch, fp16_scaler, args)
        #train_stats = train_one_epoch(student, teacher,teacher_clip, teacher_without_ddp,teacher_clip_without_ddp, ibot_loss,
        #    data_loader_train_lunkuo, optimizer, device,lr_schedule, wd_schedule, momentum_schedule,
        #    epoch, fp16_scaler, args)
        ...
        if args.output_dir and (epoch % 10 == 0 or epoch == args.epochs -1):
            utils.save_model(
                ...
                loss_scaler=ibot_loss, epoch=epoch)
...
def train_one_epoch(student, teacher,teacher_clip, teacher_without_ddp, ibot_loss, data_loader,
                    optimizer, device: torch.device,lr_schedule, wd_schedule, momentum_schedule,epoch,
                    fp16_scaler, args):

#def train_one_epoch(student, teacher,teacher_clip, teacher_without_ddp,teacher_clip_without_ddp, ibot_loss, data_loader_lunkuo,
#                    optimizer, device: torch.device,lr_schedule, wd_schedule, momentum_schedule,epoch,
#                    fp16_scaler, args):

    to_pil_image = transforms.ToPILImage() 
    save_dir = 'path_to_save_images' 
    ...
    #for  it, (images_lunkuo, _) in enumerate(metric_logger.log_every(data_loader_lunkuo, 20, header)):
    for it, (rgb_samples, tir_samples) in enumerate(metric_logger.log_every(data_loader, 20, header)):
        # update weight decay and learning rate according to their schedule
        it = len(data_loader) * epoch + it  # global training iteration
        #it = len(data_loader_lunkuo) * epoch + it  # global training iteration
        ...
        images = images.cpu()  
        images_lunkuo = images_lunkuo.cpu()  
        
        # 遍历批次中的每个图像并保存  
        for i, img_tensor in enumerate(images):  
            img_pil = to_pil_image(img_tensor)  
            img_name = f'image_{it}_{i}.png'  
            img_path = os.path.join(save_dir, img_name)  
            img_pil.save(img_path)  
            
        for i, img_tensor in enumerate(images_lunkuo):  
            img_pil = to_pil_image(img_tensor)  
            img_name = f'image_lunkuo_{it}_{i}.png'  
            img_path = os.path.join(save_dir, img_name)  
            img_pil.save(img_path)  
        
        
        with torch.cuda.amp.autocast(fp16_scaler is not None):
        #with torch.cuda.amp.autocast(metric_logger is not None):
            # get global views
            #student_output,student_loss,masks,p_x,t,tezheng = student(images, mask_ratio=args.mask_ratio)
            ...
            similarity_loss,kl_distance_loss = teacher_clip(images_lunkuo,tezheng)

            #loss1= student_loss
            student_loss = ((student_loss * masks).sum() / masks.sum())*4
            '''
            mask = masks.type(torch.long)
            
            l_s =torch.zeros(t, ).to(loss1.device)
            for i in range(p_x.shape[0]):
                ...
                # mask log-probs 掩码日志探测
                mask_log_probs = log_probs[mask[i]]

                # we need to select tokens that maximize the reconstruction error, so (-) sign
                #我们需要选择使重建误差最大化的令牌，所以（-）符号
                l_s[i] = -torch.mean(mask_log_probs*loss1[i].detach())
            
            m_l_s = 0.1*torch.mean(l_s) #Sampling loss
            '''
            # get local views
            ...
            #all_loss = ibot_loss(student_output, teacher_output, student_local_cls, masks, epoch)+student_loss
            masks = masks.type(torch.bool)
            #all_loss = ibot_loss(student_output, teacher_output, masks, epoch,student_loss,similarity_loss,kl_distance_loss,m_l_s)
            all_loss = ibot_loss(student_output, teacher_output, masks, epoch,student_loss,similarity_loss,kl_distance_loss)
...
class iBOTLoss(nn.Module):
    ...
    #def forward(self, student_output, teacher_output, student_mask, epoch,mae_loss,similarity_loss,kl_distance_loss,m_l_s):
    def forward(self, student_output, teacher_output, student_mask, epoch,mae_loss,similarity_loss,kl_distance_loss):
        ...
        #student_patch = student_output
        #teacher_patch = teacher_output
        ...
        for q in range(len(teacher_patch_c)):
            for v in range(len(student_patch_c)):
                if v == q:
                    ...
                    mask = student_mask[v]
                    mask = ~mask
                    #e1 = [token for st in loss2 for token in st ]
        ...
        #total_loss = dict( patch=total_loss2,mae =mae_loss,similarity = similarity_loss,kl = kl_distance_loss, sampling=m_l_s , loss=total_loss2+ mae_loss+m_l_s+similarity_loss+kl_distance_loss)
